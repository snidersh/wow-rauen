...
function WorldMapButton_OnUpdate()
	...
	for i=1, 10, 1 do
		...
		if ( connected ) then
			friendX, friendY = GetPlayerMapPosition("player");
			WorldMapFriendFrame = getglobal("WorldMapFriend"..i);
			if ( friendX == 0 and friendY == 0 ) then
				WorldMapFriendFrame:Hide();
			else			
				friendX = friendX * WorldMapDetailFrame:GetWidth();
				friendY = -friendY * WorldMapDetailFrame:GetHeight();
				WorldMapFriendFrame:SetPoint("CENTER", "WorldMapDetailFrame", "TOPLEFT", friendX, friendY);
				WorldMapFriendFrame:Show();
			end
		end
	end -- /Neshke
	--Position corpse
